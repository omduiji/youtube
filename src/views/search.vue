...
export default {
  ...
  methods: {
    ...
    dynamicParams.forEach(item => {
      urls.push(`${this.api.baseUrl}search?part=${this.api.part}&q=${this.$route.params.query}&type=${item.type}&regionCode=${this.api.regionCode}&maxResults=${this.api.maxResults}&key=${this.api.key}&pageToken=${item.token}`)
    })
    try {
      let result = await Promise.all(urls.map(item => fetch(item)))
      let [channelsDetails, playlistsDetails, videosDetails]= await Promise.all(result.map(item => item.json()))
      ...
      let res = [...await this.getVideos(videosDetails), ...await this.getChannels(channelsDetails), ...await this.getPlaylists(playlistsDetails)]
    ...
    async getChannels(channels) {
      ...
      let channelIds = channels.items.map(channels => channels.id.channelId).join(',')
    ...
    async getPlaylists(playlists) {
      ...
      let playlistsIds = playlists.items.map(playlists => playlists.id.playlistId).join(',')
